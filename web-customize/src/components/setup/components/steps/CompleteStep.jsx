...
const CompleteStep = ({
  ...
  const databaseType =
    setupStatus.database_type === 'sqlite'
      ...
      : setupStatus.database_type === 'mysql'
        ? 'MySQL'
        : 'PostgreSQL';
  const adminAccount = setupStatus.root_init
    ? t('已初始化')
  ...
  return (
    <div className='text-center mt-2'>
      ...
      <div className='setup-summary text-left'>
        <div className='setup-summary-item'>
          <div className='setup-summary-label'>{t('数据库类型')}</div>
          <div className='setup-summary-value'>{databaseType}</div>
        </div>
        <div className='setup-summary-item'>
          <div className='setup-summary-label'>{t('管理员账号')}</div>
          <div className='setup-summary-value'>{adminAccount}</div>
        </div>
        <div className='setup-summary-item'>
          <div className='setup-summary-label'>{t('使用模式')}</div>
          <div className='setup-summary-value'>{usageModeLabel}</div>
        </div>
      </div>
